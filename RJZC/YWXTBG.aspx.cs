...
{
    public partial class YWXTBG : System.Web.UI.Page
    {
        public string t1 = "", t0 = "";
        ...
        protected void btn_save_Click(object sender, EventArgs e)
        {
            ...
            if (this.txt_sjyq.Text.Trim() == "")//txt_sjyq
            {
            ...
            {
                str = "INSERT INTO TB_YWSJXG (ID,tbdate,rjid,syr,sybm,sjyq,bmsp,bz,userid,xzqhID,shzt) values('" + txt_bh.Text + "'," + DataOper.OleDbgetdate() + ",'" + ddl_ruanjianming.SelectedValue + "','" + txt_syr.Text + "','" + ddl_quhua.SelectedItem.Text.ToString() + " " + ddl_chushi.SelectedItem.Text.ToString() + "','" + txt_sjyq.Text + "','" + txt_bmsp.Text + "','" + txt_bz.Text + "','" + Session["userid"].ToString() + "','" + Session["XZQH"].ToString() + "','0')";
            }
            else
            {
                str = "UPDATE TB_YWSJXG SET rjid = '" + ddl_ruanjianming.SelectedValue + "',syr = '" + txt_syr.Text + "',sybm = '" + ddl_quhua.SelectedItem.Text.ToString() + " " + ddl_chushi.SelectedItem.Text.ToString() + "',bmsp = '" + txt_bmsp.Text + "',sjyq = '" + txt_sjyq.Text + "',bz = '" + txt_bz.Text + "',shzt='0'  WHERE ID = '" + this.lbl_id.Text + "'";
        ...
        {
            if (DataBase.Exe_cmd("DELETE FROM TB_YWSJXG WHERE ID ='" + GridView1.DataKeys[e.RowIndex].Value.ToString() + "'"))
            {
                //如果单位信息表中是0行的话，删除流水号表对应的信息
                if (DataBase.Exe_count("TB_YWSJXG ") == 0)
                {
