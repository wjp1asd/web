...
{
    public partial class RJMC : System.Web.UI.Page
    {
        public string t1 = "", t0 = "";
        ...
        private DataTable GetDataToTable()
        {
            ...
            dt = DataBase.Exe_dt("select * from yujin ORDER BY ID ");
            return dt;
        }
        ...
        protected void btn_add_Click(object sender, EventArgs e)
        {
            ...
            {
                ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "click", "alert('请输入预警项目和天数！');", true);
                return;
            }

            if (DataBase.Exe_count("yujin", " zclbmc='" + DataOper.setTrueString(txt_lbmc.Text.Trim()) + "' ") > 0)
            {
                ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "click", "alert('项目名称已存在！');", true);
                return;
            }
            if (DataBase.Exe_cmd("insert into yujin(Type,Times) values('"  + DataOper.setTrueString(txt_lbmc.Text.Trim()) + "','" + DataOper.setTrueString(txt_lbmcbz.Text.Trim()) + "')"))
            {
        ...
        {
            if (DataBase.Exe_cmd("DELETE FROM  yujin WHERE  ID ='" + GridView1.DataKeys[e.RowIndex].Value.ToString() + "'"))
            {
                //如果单位信息表中是0行的话，删除流水号表对应的信息
                if (DataBase.Exe_count("yujin") == 0)
                {
                    DataBase.Exe_cmd("DELETE  FROM  SYS_LSHGLB  WHERE  BM = 'yujin '");
                }
        ...
        protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            if (DataBase.Exe_cmd("update yujin set Type='" + DataOper.setTrueString(mc.Text.Trim()) + "',Times='" + DataOper.setTrueString(bz.Text.Trim()) + "' where ID='" + GridView1.DataKeys[e.RowIndex].Value.ToString() + "'"))
            {
