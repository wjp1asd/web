...
{
    public partial class YWXTSQQZ : System.Web.UI.Page
    {
        public string t1 = "", t0 = "";
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                //简拼+上年月日编号
                txt_bh.Text = "Rj" + DataOper.getlsh("TB_BGSQ", "gzrzid");
        ...
        private void getData()
        {
            ...
            this.lbl_heji.Text = DataBase.Exe_Scalar("SELECT count(*) FROM TB_BGSQ");
        ...
        private DataTable GetDataToTable()
        {
            ...
            dt = DataBase.Exe_dt("select * from (SELECT ID,tbdate,userid,rjid,(select rjname from TB_RJMC where TB_RJMC.rjid=TB_BGSQ.rjid)as rjmc,syr,sybm,gnyq,sjyq,bmsp,xxzx,bz,shzt,case when shzt='0' then '待审核' when shzt='1' then '审批结束' when shzt='2' then '驳回'  end as spshzt,xzqhID  FROM TB_BGSQ) as aa  WHERE " + ViewState["where"].ToString() + "  ORDER BY ID DESC");
            return dt;
        }
        ...
        protected void btn_add_Click(object sender, EventArgs e)
        {
            ...
            // lbl_id.Text = DataOper.getlsh("TB_BGSQ", "ID").Trim();
        ...
        {
            DataTable dt = DataBase.Exe_dt("SELECT ID,tbdate,userid,rjid,(select rjname from TB_RJMC where TB_RJMC.rjid=TB_BGSQ.rjid)as rjmc,syr,sybm,gnyq,sjyq,bmsp,xxzx,bz,shzt,case when shzt='0' then '待审核' when shzt='1' then '审批结束' when shzt='2' then '驳回'  end as spshzt,xzqhID  FROM TB_BGSQ   WHERE ID = '" + xh + "'");
            if (dt.Rows.Count > 0)
            {
        ...
        protected void btn_save_Click(object sender, EventArgs e)
        {
            ...
            {
                str = "INSERT INTO TB_BGSQ (ID,tbdate,rjid,syr,sybm,gnyq,sjyq,bmsp,bz,userid,xzqhID,shzt) values('" + txt_bh.Text + "'," + DataOper.OleDbgetdate() + ",'" + ddl_ruanjianming.SelectedValue + "','" + txt_syr.Text + "','" + ddl_quhua.SelectedItem.Text.ToString() + " " + ddl_chushi.SelectedItem.Text.ToString() + "','" + txt_gnyq.Text + "','" + txt_sjyq.Text + "','" + txt_bmsp.Text + "','" + txt_bz.Text + "','" + Session["userid"].ToString() + "','" + Session["XZQH"].ToString() + "','0')";
            }
            else
            {
                str = "UPDATE TB_BGSQ SET rjid = '" + ddl_ruanjianming.SelectedValue + "',syr = '" + txt_syr.Text + "',sybm = '" + ddl_quhua.SelectedItem.Text.ToString() + " " + ddl_chushi.SelectedItem.Text.ToString() + "',gnyq = '" + txt_gnyq.Text + "',bmsp = '" + txt_bmsp.Text + "',sjyq = '" + txt_sjyq.Text + "',bz = '" + txt_bz.Text + "',shzt='0'  WHERE ID = '" + this.lbl_id.Text + "'";
        ...
        {
            if (DataBase.Exe_cmd("DELETE FROM TB_BGSQ WHERE ID ='" + GridView1.DataKeys[e.RowIndex].Value.ToString() + "'"))
            {
                //如果单位信息表中是0行的话，删除流水号表对应的信息
                if (DataBase.Exe_count("TB_BGSQ ") == 0)
                {
                    DataBase.Exe_cmd("DELETE  FROM  SYS_LSHGLB  WHERE  BM = 'TB_BGSQ'");
                }
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                //如果提交，编辑按钮隐藏
                if (issubmit.Text == "0")//0待审核，1通过，2驳回
                {
                    bianji.Visible = false;

                }
                if (issubmit.Text == "1")//0待审核，1通过，2驳回
                {
                    bianji.Visible = false;
