...
{
    public partial class JFWHJL : System.Web.UI.Page
    {
        public string t1 = "", t0 = "";
        ...
        protected void btn_select_Click(object sender, EventArgs e)
        {
            ...
            //if (startTimes.Trim() == "")
            //{
            //    ScriptManager.RegisterStartupScript(Page, GetType(), "click", "alert('请填写开始日期！');", true);
            //    return;
            //}

            //if (endTimes.Trim() == "")
            //{
            //    ScriptManager.RegisterStartupScript(Page, GetType(), "click", "alert('请填写截止日期！');", true);
            //    return;
            //}
        ...
        protected void btn_save_Click(object sender, EventArgs e)
        {
            ...
            string sbmc = txt_sbmc.Text;//设备名称
            ...
            if (ViewState["edit"].ToString() == "add")
            {
                ...
                try
                {
                    if (DataBase.Exe_cmd("insert into TB_JFWHJL(ID,whrq,sbmc,sbmcid,jcdw,jcry,whptry,djr,xzqhID,yianyi,sign) values('" + this.lbl_id.Text + "','" + whrq + "','" + sbmc + "','" + sbmcid + "','" + jcdw + "','" + jcry + "','" + ptry + "','" + Session["USERNAME"].ToString() + "','" + Session["XZQH"].ToString() + "','" + txt_yijian.Text.Trim() + "','1')"))
                    {
        ...
        private void getData()
        {
            ...
            this.lbl_heji.Text = DataBase.Exe_Scalar("select COUNT(ID) FROM TB_JFWHJL WHERE xzqhID='" + Session["XZQH"].ToString() + "'  and   sign='1' ");
        }
        ...
        private DataTable GetDataToTable()
        {
            ...
            dt = DataBase.Exe_dt("select ID,whrq,sbmc,sbmcid,jcdw,jcry,(select lfname from TB_JFCRSQ where TB_JFCRSQ.ID=TB_JFWHJL.jcry)as lfname,(select username from sys_user where code=whptry )as whptry,djr,yianyi,xzqhID from TB_JFWHJL  WHERE   xzqhID='" + Session["XZQH"].ToString() + "'  and   sign='1' " + ViewState["where"].ToString() + "  ORDER BY ID ");
            return dt;
        }
